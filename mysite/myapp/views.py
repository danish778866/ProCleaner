...
def list(request):
    ...
    if CLIENT_TOKEN_KEY not in request.session:
        if current_url.endswith("/list/"):
            ...
            request.session[CLIENT_TOKEN_KEY] = token_response.json()['access_token']
            request.session.set_expiry(token_response.json()['expires_in'])
...
def list_tuples(request):
    ...
    #if request.POST['process_file'] == '' and 'docfile' not in request.FILES:
    ...
    if True:
        ...
        if upload_type == "CDrive":
            ...
            sample_tuples = A['foo'].head().tolist()
        response_data = {}
        response_data['sample_tuples'] = sample_tuples
